...
class _Unet(nn.Module):
    ...
    def _setup(self):
        ...
        # encoder
        self.enc_conv_blocks = nn.ModuleList(
            [self._setup_conv_block(self.in_chans, self.chans)]
        )
        ch = self.chans
        ...
        for _ in range(self.num_pool_layers):
            self.dec_transpose_convs.append(
                self._setup_transpose_conv_block(ch, ch // 2)
            )
    ...
    @abstractmethod
    def _setup_transpose_conv_block(
    ...
    def complex_to_chan_dim(x: torch.Tensor):
        if os.environ.get('ONNX_EXPORT', False) == 'True':
            # torch.movedim is not supported by ONNX export
            b, c = x.shape[:2]
            thw = x.shape[2:-1]
            x = x.permute(
                0, x.ndim - 1, *range(1, x.ndim - 1)
            )  # [b, 2, c, (t), h, w]
            x = x.reshape((b, 2 * c) + thw)  # [b, 2*c, (t), h, w]
        else:
    ...
    def chan_complex_to_last_dim(x: torch.Tensor) -> torch.Tensor:
        if os.environ.get('ONNX_EXPORT', False) == 'True':
            ...
            b, c2 = x.shape[:2]
            thw = x.shape[2:]
            x = x.reshape((b, 2, c2 // 2) + thw)  # [b, 2, c, (t), h, w]
            x = x.permute(0, *range(2, x.ndim), 1)  # [b, c, (t), h, w, 2]
        else:
    ...
    def forward(self, image: torch.Tensor) -> torch.Tensor:
        ...
        # decoder
        for transpose_conv, conv_block in zip(
            self.dec_transpose_convs, self.dec_conv_blocks
        ):
            # upsample
            ...
            # reflect pad on the right/bottom/(back) if needed to handle odd
            # input dimensions
            padding = (
                0,
                enc_features.shape[-1] - output.shape[-1],  # padding right
                0,
                enc_features.shape[-2] - output.shape[-2],  # padding bottom
            )
            if output.ndim == 5:
                padding += (
                    0,
                    enc_features.shape[-3] - output.shape[-3],
                )  # padding back
...
class Unet2d(_Unet):
    ...
    def _setup_padder(self) -> nn.Module:
        ...
        )  # tensor size should be at least two at the bottleneck
    ...
    def _setup_conv_block(self, in_chans: int, out_chans: int) -> nn.Module:
        return ConvBlock2d(
            ...
            two_convs_per_block=self.two_convs_per_block,
        )
    ...
    def forward(self, image: torch.Tensor) -> torch.Tensor:
        ...
        if len(in_shape) == 6:
            image = image.reshape(
                (-1, in_shape[2], in_shape[1], *in_shape[-3:])
            )  # [b, t, c, h, w, two]
...
class _UnetSpatiotemporal(_Unet):
    ...
    def _setup_padder(self) -> nn.Module:
        ...
        return Padder(
            size_multiple_of=size_multiple_of, pad_temporal=self.pool_temporal
        )
    ...
    def _pool(self, x: torch.Tensor) -> torch.Tensor:
        if self.pool_temporal:
            ...
            return func.avg_pool3d(
                x, kernel_size=(1, 2, 2), stride=(1, 2, 2), padding=0
            )


class Unet2dPlusTime(_UnetSpatiotemporal):
    def _setup_conv_block(self, in_chans: int, out_chans: int) -> nn.Module:
        return ConvBlock2dPlusTime(
            in_chans,
            out_chans,
            self.drop_prob,
            conv_size=self.conv_size,
            conv_size_temp=self.conv_size_temp,
    ...
    def _setup_transpose_conv_block(
        ...
        return TransposeConvBlock2dPlusTime(
            in_chans,
            out_chans,
            upsample_temporal=self.pool_temporal,
            pad_mode_temp=self.pad_mode_temp,
        )
...
class _ConvBlock(nn.Module):
    def __init__(
        ...
        self.act_args = {'negative_slope': 0.2, 'inplace': True}
    ...
    def forward(self, image: torch.Tensor) -> torch.Tensor:
        ...
        if self.residual_blocks:
            ...
            ), 'Identity and out must be set for residual connection.'
...
class ConvBlock2d(_ConvBlock):
    def __init__(self, *args, conv_size: int = 3, **kwargs):
        ...
        if conv_size % 2 != 1 or conv_size <= 0:
            raise ValueError(
                f'`conv_size` must be a positive odd integer, but was {conv_size}'
            )
    ...
    def _setup_layers(self) -> None:
        layers = [
            conv.Conv2dSpatial(
                in_channels=self.in_channels, **self.conv_args_spatial
            ),
        ...
        if self.two_convs_per_block:
            layers.extend(
                [
                    ...
                    conv.Conv2dSpatial(
                        in_channels=self.out_channels, **self.conv_args_spatial
                    ),
                    nn.InstanceNorm2d(self.out_channels),
                ]
            )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if conv_size_temp % 2 != 1 or conv_size_temp <= 0:
            raise ValueError(
                f'`conv_size_temp` must be a positive odd integer, but was {conv_size_temp}'
            )
...
class ConvBlock2dPlusTime(_ConvBlockSpatiotemporal):
    def _setup_layers(self) -> None:
        conv1 = nn.Sequential(
            ...
            nn.LeakyReLU(**self.act_args),
            conv.Conv3dTemporal(
                in_channels=self.out_channels, **self.conv_args_temporal
            ),
        )
        layers = [
            conv1,
...
class ConvBlock3d(_ConvBlockSpatiotemporal):
    def _setup_layers(self) -> None:
        layers = [
            conv.Conv3dSpatiotemporal(
                in_channels=self.in_channels, **self.conv_args_spatiotemporal
            ),
            nn.InstanceNorm3d(self.out_channels),
        ]
        if self.two_convs_per_block:
...
class TransposeConvBlock2d(_TransposeConvBlock):
    ...
    def _setup_layers(self) -> None:
        self.layers = nn.Sequential(
            # Upsample + Conv{2,3}d reduces checkerboard artifacts compared to
            # ConvTranspose{2,3}d https://distill.pub/2016/deconv-checkerboard/
...
class TransposeConvBlock2dPlusTime(_TransposeConvBlock):
    ...
    def _setup_layers(self) -> None:
        self.layers = nn.Sequential(
            nn.Upsample(
                scale_factor=(2 if self.upsample_temporal else 1, 2, 2)
            ),
            conv.Conv3dSpatial(
                in_channels=self.in_channels, **self.conv_args_spatial
            ),
...
class Padder(nn.Module):
    ...
    def _pad_circular(
        self, x: torch.Tensor, pad: Tuple[int, ...], dim_from_right: int
    ) -> torch.Tensor:
        pad_ = (
            (0, 0) * dim_from_right
            + pad
            + (0, 0) * (x.ndim - dim_from_right - 3)
        )
        x_padded = torch.nn.functional.pad(x, pad_, mode='constant')
        dim_from_left = x.ndim - dim_from_right - 1
        len_dst = x_padded.shape[dim_from_left]
        len_src = x.shape[dim_from_left]
        dst_front = (
            [slice(None)] * dim_from_left
            + [slice(0, pad[0])]
            + [slice(None)] * dim_from_right
        )
        dst_back = (
            [slice(None)] * dim_from_left
            + [slice(len_dst - pad[1], len_dst)]
            + [slice(None)] * dim_from_right
        )
        src_front = (
            [slice(None)] * dim_from_left
            + [slice(len_src - pad[0], len_src)]
            + [slice(None)] * dim_from_right
        )
        src_back = (
            [slice(None)] * dim_from_left
        ...
        x_padded[tuple(dst_front)] = x[tuple(src_front)]
        x_padded[tuple(dst_back)] = x[tuple(src_back)]
        return x_padded
    ...
    def pad(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if isinstance(x.shape[0], int):
            ...
            # this is the case during onnx export in tracing mode (elements of torch.Size are torch.Tensors)
            unpadded_size = torch.stack([*x.shape]).to(torch.int64)  # type: ignore
            padded_size = torch.stack([*x.shape]).to(torch.int64)  # type: ignore
        for dim in range(3 if self.pad_temporal else 2):
            padded_size[-dim - 1] = (
                (x.shape[-dim - 1] - 1) // self.size_multiple_of + 1
            ) * self.size_multiple_of
        padding_front = torch.floor((padded_size - unpadded_size) / 2).to(
            torch.int64
        )
        ...
        if os.environ.get('ONNX_EXPORT', False) == 'True':
            # circular padding is only supported in ONNX starting from opset_version 19, hence we implement it manually here
            x = self._pad_circular(x, (padding_front[-1], padding_back[-1]), dim_from_right=0)  # type: ignore
        else:
        ...
        if self.pad_temporal:
            if os.environ.get('ONNX_EXPORT', False) == 'True':
                # circular padding is only supported in ONNX starting from opset_version 19, hence we implement it manually here
                x = self._pad_circular(x, (padding_front[-3], padding_back[-3]), dim_from_right=2)  # type: ignore
            else:
...
class NormUnet(nn.Module):
    ...
    def norm(
        ...
        std = (
            x.flatten(1, -2).std(dim=1).view((-1,) + (1,) * (x.ndim - 2) + (2,))
        )
