...
class ConvNdWithFlexiblePadding(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if len(padding) != N:
            raise ValueError(
                f'`padding` must have length {N}, but was {len(padding)}'
            )
        if len(padding_modes) != N:
            raise ValueError(
                f'`padding_modes` must have length {N}, but was {len(padding_modes)}'
            )
        ...
        match N:
            ...
            case _:
                raise NotImplementedError(
                    f'ConvNdWithFlexiblePadding only implemented for N={{1, 2, 3}}, but was {N}'
                )
    ...
    def pad(self, image: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(self.N):
            ...
            if (
                os.environ.get('ONNX_EXPORT', False) == 'True'
                and mode == 'circular'
            ):
                ...
                image_padded = torch.nn.functional.pad(
                    image, pad, mode='constant'
                )
                i_ = image.ndim - i - 1
                len_dst = image_padded.shape[i_]
                len_src = image.shape[i_]
                dst_front = (
                    [slice(None)] * i_ + [slice(0, p)] + [slice(None)] * i
                )
                dst_back = (
                    [slice(None)] * i_
                    + [slice(len_dst - p, len_dst)]
                    + [slice(None)] * i
                )
                src_front = (
                    [slice(None)] * i_
                    + [slice(len_src - p, len_src)]
                    + [slice(None)] * i
                )
                src_back = (
                    [slice(None)] * i_ + [slice(0, p)] + [slice(None)] * i
                )
                image_padded[tuple(dst_front)] = image[tuple(src_front)]
                image_padded[tuple(dst_back)] = image[tuple(src_back)]
            else:
...
class Conv3dSpatiotemporal(ConvNdWithFlexiblePadding):
    def __init__(
        self, padding: Tuple[int, int, int], pad_mode_temp: str, **kwargs
    ):
        # `pad_mode_temp` in temporal dimension, 'replicate' in readout dimension, 'circular' in phase dimension
        super().__init__(
            N=3,
            padding=padding,
            padding_modes=(pad_mode_temp, 'replicate', 'circular'),
            **kwargs,
        )
...
class Conv1dTemporal(ConvNdWithFlexiblePadding):
    def __init__(self, padding: Tuple[int,], pad_mode_temp: str, **kwargs):
        ...
        super().__init__(
            N=1, padding=padding, padding_modes=(pad_mode_temp,), **kwargs
        )
